...
function(generate_arduino_firmware INPUT_NAME)
    ...
    parse_generator_arguments(${INPUT_NAME} INPUT
                              ...
                              "SRCS;HDRS;LIBS;ARDLIBS;AFLAGS"  # Multi Value Keywords
                              ${ARGN})

    set(INPUT_BOARD ${ARDU_BOARD_ID})
...
function(REGISTER_HARDWARE_PLATFORM PLATFORM_PATH)  
    ...
    message(STATUS "REGISTER_HARDWARE_PLATFORM ${PLATFORM}")
    ...
    if(PLATFORM)
        ...
        if (platform_exists EQUAL -1)
            ...
            message(STATUS "${PLATFORM}_CORES_PATH= ${${PLATFORM}_CORES_PATH}")
            message(STATUS "${PLATFORM}_VARIANTS_PATH= ${${PLATFORM}_VARIANTS_PATH}")
            message(STATUS "${PLATFORM}_BOOTLOADERS_PATH= ${${PLATFORM}_BOOTLOADERS_PATH}")
            message(STATUS "${PLATFORM}_PROGRAMMERS_PATH= ${${PLATFORM}_PROGRAMMERS_PATH}")
            message(STATUS "${PLATFORM}_BOARDS_PATH= ${${PLATFORM}_BOARDS_PATH}")
            ...
            message(WARNING "REGISTER_HARDWARE_PLATFORM skipping ${PLATFORM} -- already registered!")
        endif()    
    endif()
...
#       MANUAL - (Advanced) Only use Libc/Includes
#
...
function(setup_arduino_target TARGET_NAME BOARD_ID ALL_SRCS ALL_LIBS COMPILE_FLAGS LINK_FLAGS MANUAL)
    ...
    set_target_properties(${TARGET_NAME} PROPERTIES
                ...
                LINK_FLAGS "${ARDUINO_LINK_FLAGS} ${LINK_FLAGS}")
    target_link_libraries(${TARGET_NAME}  -Wl,--start-group ${ALL_LIBS} ${ARDU_LINKER_LIBS} -Wl,--end-group)
    ...
    if (ARDU_RECIPE_EEP AND (NOT " " STREQUAL ARDU_RECIPE_EEP))
        add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
                            COMMAND ${CMAKE_OBJCOPY}
                            ARGS     ${ARDUINO_OBJCOPY_EEP_FLAGS}
                                     ${TARGET_PATH}.elf
                                     ${TARGET_PATH}.eep
                            COMMENT "Generating EEPROM image from elf"
                            VERBATIM)
    endif()

    # Convert firmware image to ASCII HEX format
    if (ARDU_RECIPE_HEX AND (NOT " " STREQUAL ARDU_RECIPE_HEX))

        # prepare cmd from recipe
        string(REGEX MATCH "\"[^\"]*\"" HEX_COMMAND ${ARDU_RECIPE_HEX})
        string(REPLACE "${HEX_COMMAND}" "" HEX_ARGS ${ARDU_RECIPE_HEX})
        string(REPLACE "\"" "" HEX_COMMAND ${HEX_COMMAND})
    
        # prepare args from recipe
        string(REPLACE "<TARGET>" "${TARGET_NAME}" HEX_ARGS ${HEX_ARGS})
        string(REPLACE "<CMAKE_CURRENT_BINARY_DIR>" "${CMAKE_CURRENT_BINARY_DIR}" HEX_ARGS ${HEX_ARGS})
        string(STRIP ${HEX_ARGS} HEX_ARGS)
        string(REPLACE "\"" "" HEX_ARGS ${HEX_ARGS})
        string(REPLACE " " ";" HEX_ARGS ${HEX_ARGS})
        add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
                            COMMAND ${HEX_COMMAND}
                            ARGS ${HEX_ARGS}
                            COMMENT "Generating HEX image from elf"
                            VERBATIM)
    endif()
    

    # Display target size
    if (ARDU_RECIPE_SIZE AND (NOT " " STREQUAL ARDU_RECIPE_SIZE))
        # prepare cmd from recipe
        string(REGEX MATCH "\"[^\"]*\"" SIZE_COMMAND ${ARDU_RECIPE_SIZE})
        string(REPLACE "${SIZE_COMMAND}" "" SIZE_ARGS ${ARDU_RECIPE_SIZE})
        string(REPLACE "\"" "" SIZE_COMMAND ${SIZE_COMMAND})
    
        # prepare args from recipe
        string(REPLACE "<TARGET>" "${TARGET_NAME}" SIZE_ARGS ${SIZE_ARGS})
        string(REPLACE "<CMAKE_CURRENT_BINARY_DIR>" "${CMAKE_CURRENT_BINARY_DIR}" SIZE_ARGS ${SIZE_ARGS})
        string(STRIP ${SIZE_ARGS} SIZE_ARGS)
        string(REPLACE "\"" "" SIZE_ARGS ${SIZE_ARGS})
        string(REPLACE " " ";" SIZE_ARGS ${SIZE_ARGS})
        add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
    ...
    # Upload image target
    if (ARDU_RECIPE_UPLOAD AND (NOT " " STREQUAL ARDU_RECIPE_UPLOAD))
        # prepare cmd from recipe
        string(REGEX MATCH "\"[^\"]*\"" UPLOAD_COMMAND ${ARDU_RECIPE_UPLOAD})
        string(REPLACE "${UPLOAD_COMMAND}" "" UPLOAD_ARGS ${ARDU_RECIPE_UPLOAD})
        string(REPLACE "\"" "" UPLOAD_COMMAND ${UPLOAD_COMMAND})
    
        # prepare args from recipe
        string(REPLACE "<TARGET>" "${TARGET_NAME}" UPLOAD_ARGS ${UPLOAD_ARGS})
        string(REPLACE "<CMAKE_CURRENT_BINARY_DIR>" "${CMAKE_CURRENT_BINARY_DIR}" UPLOAD_ARGS ${UPLOAD_ARGS})
        string(STRIP ${UPLOAD_ARGS} UPLOAD_ARGS)
        string(REPLACE "\"" "" UPLOAD_ARGS ${UPLOAD_ARGS})
        string(REPLACE " " ";" UPLOAD_ARGS ${UPLOAD_ARGS})
...
function(LOAD_ARDUINO_STYLE_SETTINGS SETTINGS_LIST SETTINGS_PATH)
    ...
    message(STATUS "LOAD_ARDUINO_STYLE_SETTINGS to ${SETTINGS_LIST} from ${SETTINGS_PATH}")
...
# toolchain specific init that has to be done 'late'
toolchain_init()

if(NOT ARDUINO_FOUND AND ARDUINO_SDK_PATH)

    set(HARDWARE_PLATFORM_PATH ${ARDU_PLATFORM_PATH})
    if(NOT HARDWARE_PLATFORM_PATH)
        set(HARDWARE_PLATFORM_PATH ${ARDUINO_SDK_PATH}/hardware/arduino/)
        message(STATUS "HARDWARE_PLATFORM_PATH not given, will assume ${HARDWARE_PLATFORM_PATH}")
    endif()

    message(STATUS "HARDWARE_PLATFORM_PATH=${HARDWARE_PLATFORM_PATH}")
    GET_FILENAME_COMPONENT(PLATFORM "${ARDU_PLATFORM_PATH}" NAME)
    register_hardware_platform(${HARDWARE_PLATFORM_PATH})

    find_file(ARDUINO_LIBRARIES_PATH
        NAMES libraries
        PATHS ${HARDWARE_PLATFORM_PATH}
    ...
    required_variables(VARS
        ...
        ${${PLATFORM}_LIBRARIES_PATH}
        ${${PLATFORM}_BOARDS_PATH}
        ${${PLATFORM}_PROGRAMMERS_PATH}
        ${${PLATFORM}_VARIANTS_PATH}
        ARDUINO_PLATFORMS
        ARDUINO_VERSION_PATH
        MSG "Invalid HARDWARE_PLATFORM_PATH path (${HARDWARE_PLATFORM_PATH}).\n")

    message(STATUS "Arduino SDK: ${ARDUINO_SDK_PATH}")
...
#                              C Flags                                        
#=============================================================================#
set(CMAKE_C_FLAGS                "-g -Os       "    CACHE STRING "")
...
#                             C++ Flags                                       
#=============================================================================#
set(CMAKE_CXX_FLAGS                "-g -Os       " CACHE STRING "")
...
set(CMAKE_EXE_LINKER_FLAGS                "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_EXE_LINKER_FLAGS_DEBUG          "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_EXE_LINKER_FLAGS_MINSIZEREL     "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_EXE_LINKER_FLAGS_RELEASE        "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_EXE_LINKER_FLAGS_RELWITHDEBINFO "${ARDU_LINKER_FLAGS}" CACHE STRING "")

#=============================================================================#
#=============================================================================#
#                       Shared Lbrary Linker Flags                            #
#=============================================================================#
set(CMAKE_SHARED_LINKER_FLAGS                "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_SHARED_LINKER_FLAGS_DEBUG          "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_SHARED_LINKER_FLAGS_MINSIZEREL     "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_SHARED_LINKER_FLAGS_RELEASE        "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO "${ARDU_LINKER_FLAGS}" CACHE STRING "")

set(CMAKE_MODULE_LINKER_FLAGS                "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_MODULE_LINKER_FLAGS_DEBUG          "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_MODULE_LINKER_FLAGS_MINSIZEREL     "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_MODULE_LINKER_FLAGS_RELEASE        "${ARDU_LINKER_FLAGS}" CACHE STRING "")
set(CMAKE_MODULE_LINKER_FLAGS_RELWITHDEBINFO "${ARDU_LINKER_FLAGS}" CACHE STRING "")
